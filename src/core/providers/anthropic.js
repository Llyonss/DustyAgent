...
function createStream(prompt, signal, extra) {
  ...
  if (!client || key !== k) {
    client = new Anthropic({ apiKey: process.env.LLM_API_KEY, baseURL: process.env.LLM_BASE_URL });
    key = k;
  }
  return client.messages.create({
    model: process.env.LLM_MODEL, max_tokens: 64000, eager_input_streaming: true,
